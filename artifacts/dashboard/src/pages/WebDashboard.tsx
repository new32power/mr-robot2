  ...
  return (
    <div style={{ padding: 10, display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <div style={{ background: t.card, borderRadius: 10, border: `1px solid ${t.cardB}`, overflow: "hidden" }}>
        ...
        {sessions.length === 0
          ? <div style={{ padding: 20, textAlign: "center", color: "#94a3b8", fontSize: 12 }}>No active session</div>
          : sessions.map((s, idx) => {
              const isCurrent = s.id === mySessionId;
              return (
                <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: isCurrent ? (t === DT ? "#2e1f5e" : "#f5f3ff") : "transparent", borderBottom: idx < sessions.length - 1 ? `1px solid ${t.cardB}` : "none" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                      ...
                      {isCurrent && <span style={{ background: t.accent, color: "#fff", borderRadius: 99, padding: "1px 6px", fontSize: 9, fontWeight: 800 }}>THIS DEVICE</span>}
                    </div>
                    <div style={{ fontSize: 10, color: "#94a3b8", marginTop: 1 }}>Login: {fmtTime(s.loginTime)}</div>
                  </div>
...
function LoginPage({ onAuth, appId, appName, panelToken }: { onAuth: () => void; appId: string; appName: string; panelToken: string }) {
  ...
  async function sendComplaint() {
    ...
    try {
      // Bot token stays server-side — worker sends to Telegram
      await apiFetch(`/api/apps/${encodeURIComponent(appId)}/submit-complaint`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text: complaintText.trim(), lang: complaintLang ?? "english" }),
      });
    ...
    <div style={{
      ...
      {showComplaint&&(
        <div style={{position:"fixed",inset:0,zIndex:99999,background:"#0a0f1a",
          display:"flex",flexDirection:"column",fontFamily:"system-ui,-apple-system,'Segoe UI',sans-serif"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              <div style={{width:44,height:44,borderRadius:13,flexShrink:0,
                ...
                <svg width="26" height="26" viewBox="0 0 34 34" fill="none">
                  <line x1="17" y1="1" x2="17" y2="7" stroke="#a5b4fc" strokeWidth="1.8" strokeLinecap="round"/>
                  <circle cx="17" cy="1.5" r="2" fill="#a5b4fc"/>
                  ...
                  <rect x="2" y="16" width="2" height="5" rx="1" fill="#4338ca"/>
                  <rect x="30" y="16" width="2" height="5" rx="1" fill="#4338ca"/>
                  <rect x="8" y="22" width="18" height="4" rx="1.5" fill="#1e1b4b"/>
              ...
              <div>
                <div style={{fontWeight:800,fontSize:15,color:"#f1f5f9",letterSpacing:0.2}}>Support Bot</div>
                <div style={{display:"flex",alignItems:"center",gap:5,marginTop:3}}>
                  <div style={{width:7,height:7,borderRadius:"50%",background:"#4ade80",boxShadow:"0 0 6px #4ade8099"}}/>
                  <span style={{fontSize:11,color:"#4ade80",fontWeight:600,letterSpacing:0.3}}>Online — Ready to Help</span>
                </div>
              </div>
            </div>
            <button onClick={()=>setShowComplaint(false)}
              style={{width:36,height:36,borderRadius:10,background:"#1e293b",border:"1px solid #334155",
                color:"#94a3b8",cursor:"pointer",padding:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
