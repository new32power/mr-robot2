...
app.use("*", cors({
  ...
  allowHeaders: ["Content-Type", "Authorization", "x-master-pin", "x-api-key", "x-session-token", "x-app-id", "x-silent", "x-requested-with", "x-height", "x-width"],
}));
...
app.patch("/api/devices/:deviceId", async (c) => {
  ...
  const hasAdminFields = body.starred !== undefined; // forwardEnabled/forwardSlot allowed from Android device without admin session
  if (hasAdminFields) {
